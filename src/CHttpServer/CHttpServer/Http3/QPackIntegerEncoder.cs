...
{
    public bool TryEncode(Span<byte> destination, long number, byte prefixLength, out int writtenLength)
    {
    ...
    public bool TryEncodeSimd(Span<byte> destination, int number, byte prefixLength, out int writtenCount)
    {
        //if I < 2 ^ N - 1, encode I on N bits
        //else
        //    encode(2 ^ N - 1) on N bits
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public bool TryEncodeSimd(Span<byte> destination, long number, byte prefixLength, out int writtenCount)
    {
        //if I < 2 ^ N - 1, encode I on N bits
        //else
        //    encode(2 ^ N - 1) on N bits
