...
internal class FeatureCollectionLookupContext<TContext> : FeatureCollectionLookup, IHostContextContainer<TContext> where TContext : notnull
{
    ...
    protected override FeatureCollectionLookupContext<TContext> Create() => new FeatureCollectionLookupContext<TContext>();
}

public class FeatureCollectionLookup : IFeatureCollection
{
    ...
    internal FeatureCollectionLookup Copy()
    {
    ...
    internal FeatureCollectionLookup ToContextAware<TContext>() where TContext : notnull
    {
        Debug.Assert(_checkpointRevision == 0);
        var copy = new FeatureCollectionLookupContext<TContext>();
        foreach (var pair in _features)
            copy._features.Add(pair.Key, pair.Value);
        _revision++;
        return copy;
    ...
    protected virtual FeatureCollectionLookup Create() => new FeatureCollectionLookup();
